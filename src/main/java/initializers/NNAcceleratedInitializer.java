...
public class NNAcceleratedInitializer implements Initializer {
	...
	public NNAcceleratedInitializer() {
		this.loadModel();
	}

	public NNAcceleratedInitializer(Map map) {
		this.loadModel();
		this.map = map;
	}
	...
	public void loadModel() {
		...
		String json = "";
		try {
			json = FileUtils.readFileToString(new File("nnmodels/32x32.json"), "utf-8");
		} catch (Exception e) {
			e.printStackTrace();
		}

		this.model.initializeNetwork(json);
	...
	public List<Correspondence2D2D> registerData(long frameNum, String frameTitle, ImageData imageData,
		...
		if (frameNum % MAX_FRAMES == 0) {
			...
			boolean referenceFrameUpdated = false;
			if (cs.meanDisparity > Parameters.<Integer>get("width") * 0.035) {
				this.referenceFrame = imageData;
				this.referenceTitle = frameTitle;
				referenceFrameUpdated = true;
			}

			if ((!Parameters.<Boolean>get("smartInitialization") || initializationSuitable(cs))
					&& !referenceFrameUpdated) {
				...
				Utils.pl("Initialization deemed unsuitable on this frame (" + frameNum + ").");
			}
	...
	public boolean initializationSuitable(CorrespondenceSummary cs) {
		double[] data = cs.getArray();
		Matrix input = new Matrix(data, 1);
		Matrix result = this.model.feedForward(input);
		return result.get(0, 0) > 0.5;
	}
